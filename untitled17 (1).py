...
st.title("⛏️ Borehole Data Analysis App")

uploaded_file = st.file_uploader("📂 Upload Borehole Excel (91 BH dataset)", type=["xlsx"])
...
if uploaded_file is not None:
    ...
    # Expecting columns: Borehole | Lithology | Length | Al2O3 | SiO2
    st.success("✅ Data loaded successfully!")

    # -----------------------
    # Page Navigation
    # -----------------------
    page = st.sidebar.radio("📑 Select Page", ["Borehole Selection", "Lithology Breakdown", "Overall Quality"])

    # -----------------------
    # Page 1: Borehole Selection
    # -----------------------
    if page == "Borehole Selection":
        st.header("🔎 Select Borehole(s)")
    ...
    # -----------------------
        ...
        st.header("📊 Overall Quality of Selected Boreholes")
        boreholes = df['borehole'].unique()
        ...
        if selected_bhs:
            ...
            st.metric("Weighted Avg Al₂O₃", f"{weighted_al2o3:.2f} %")
            st.metric("Weighted Avg SiO₂", f"{weighted_sio2:.2f} %")

else:
    st.info("👆 Please upload your Borehole dataset (Excel) to continue.")